...
layui.use(['laydate','admin','form', 'jquery', 'table', 'js_tools'], function () {
    ...
    option.url = "/admin/api/tieCmt/page";
    option.cols = [[
        {fixed: 'left',checkbox: true},
        {field: 'id', title: '评论id'},
        {field: 'cmtId', title: '回复id'},
        {field: 'tieId', title: '帖子id'},
    ...
    window.tieCmtListIns = table.render(option);
    ...
    table.on('tool(list)', function (obj) {
        ...
        if (obj.event === 'edit') {
            // 编辑功能
            WeAdminEdit('编辑','./tieCmtEdit.html',data.id);
        } else if (obj.event === 'del') {
            //删除功能
            js_tools.quick_post("/admin/api/tieCmt/delete",data,function (rs) {
    ...
    form.on('submit(search)', function(data){
        ...
        tieCmtListIns.reload({where:data.field});
