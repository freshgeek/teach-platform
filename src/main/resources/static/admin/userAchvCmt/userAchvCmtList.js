...
layui.use(['laydate','admin','form', 'jquery', 'table', 'js_tools'], function () {
    ...
    option.url = "/admin/api/userAchvCmt/page";
    option.cols = [[
    ...
    window.userAchvCmtListIns = table.render(option);
    ...
    table.on('tool(list)', function (obj) {
        ...
        if (obj.event === 'edit') {
            // 编辑功能
            WeAdminEdit('编辑','./userAchvCmtEdit.html',data.id);
        } else if (obj.event === 'del') {
            //删除功能
            js_tools.quick_post("/admin/api/userAchvCmt/delete",data,function (rs) {
    ...
    form.on('submit(search)', function(data){
        ...
        userAchvCmtListIns.reload({where:data.field});
