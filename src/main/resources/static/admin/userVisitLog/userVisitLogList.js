...
layui.use(['laydate','admin','form', 'jquery', 'table', 'js_tools'], function () {
    ...
    table.on('tool(list)', function (obj) {
        ...
        if (obj.event === 'edit') {
            // 编辑功能
            WeAdminEdit('编辑','./userVisitLogEdit.html',data.id);
        } else if (obj.event === 'del') {
            //删除功能
            js_tools.quick_post("/admin/api/userVisitLog/delete",data,function (rs) {
    ...
    form.on('submit(search)', function(data){
        ...
        userVisitLogListIns.reload({where:data.field});
