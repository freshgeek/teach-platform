...
layui.use(['laydate','admin','form', 'jquery', 'table', 'js_tools'], function () {
    ...
    option.url = "/admin/api/userRole/page";
    option.cols = [[
    ...
    window.userRoleListIns = table.render(option);
    ...
    table.on('tool(list)', function (obj) {
        ...
        if (obj.event === 'edit') {
            // 编辑功能
            WeAdminEdit('编辑','./userRoleEdit.html',data.id);
        } else if (obj.event === 'del') {
            //删除功能
            js_tools.quick_post("/admin/api/userRole/delete",data,function (rs) {
    ...
    form.on('submit(search)', function(data){
        ...
        userRoleListIns.reload({where:data.field});
