...
layui.use(['laydate','admin','form', 'jquery', 'table', 'js_tools'], function () {
    ...
    option.url = "/admin/api/userTip/page";
    option.cols = [[
        {fixed: 'left',checkbox: true},
        {field: 'id', title: '动态id'},
        {field: 'resourceUserId', title: '来源用户id'},
        {field: 'userId', title: '提醒用户id'},
    ...
    window.userTipListIns = table.render(option);
    ...
    table.on('tool(list)', function (obj) {
        ...
        if (obj.event === 'edit') {
            // 编辑功能
            WeAdminEdit('编辑','./userTipEdit.html',data.id);
        } else if (obj.event === 'del') {
            //删除功能
            js_tools.quick_post("/admin/api/userTip/delete",data,function (rs) {
    ...
    form.on('submit(search)', function(data){
        ...
        userTipListIns.reload({where:data.field});
