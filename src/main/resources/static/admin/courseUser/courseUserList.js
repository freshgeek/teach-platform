...
layui.use(['laydate','admin','form', 'jquery', 'table', 'js_tools'], function () {
    ...
    option.url = "/admin/api/courseUser/page";
    option.cols = [[
    ...
    window.courseUserListIns = table.render(option);
    ...
    table.on('tool(list)', function (obj) {
        ...
        if (obj.event === 'edit') {
            // 编辑功能
            WeAdminEdit('编辑','./courseUserEdit.html',data.id);
        } else if (obj.event === 'del') {
            //删除功能
            js_tools.quick_post("/admin/api/courseUser/delete",data,function (rs) {
    ...
    form.on('submit(search)', function(data){
        ...
        courseUserListIns.reload({where:data.field});
