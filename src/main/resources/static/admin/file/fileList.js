...
layui.use(['laydate','admin','form', 'jquery', 'table', 'js_tools'], function () {
    ...
    option.url = "/admin/api/file/page";
    option.cols = [[
    ...
    window.fileListIns = table.render(option);
    ...
    table.on('tool(list)', function (obj) {
        ...
        if (obj.event === 'edit') {
            // 编辑功能
            WeAdminEdit('编辑','./fileEdit.html',data.id);
        } else if (obj.event === 'del') {
            //删除功能
            js_tools.quick_post("/admin/api/file/delete",data,function (rs) {
    ...
    form.on('submit(search)', function(data){
        ...
        fileListIns.reload({where:data.field});
