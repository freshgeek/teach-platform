...
layui.use(['laydate','admin','form', 'jquery', 'table', 'js_tools'], function () {
    ...
    //执行一个laydate实例
    laydate.render({
        elem: '#signDateStart' //指定元素
        ,type: 'datetime'
    });

    //执行一个laydate实例
    laydate.render({
        elem: '#signDateEnd' //指定元素
        ,type: 'datetime'
    });

    var option = js_tools.getTableTemplate();
    option.url = "/admin/api/userSign/page";
    option.cols = [[
        {fixed: 'left',checkbox: true},
        {field: 'id', title: '未备注字段'},
        {field: 'userId', title: '未备注字段'},
        {field: 'createTime', title: '未备注字段'},
        {field: 'signDate', title: '未备注字段'},
        {fixed: 'right',title:'操作', width:150, align:'center', toolbar: '#list_btn'} //这里的toolbar值是模板元素的选择器
    ]];

    window.userSignListIns = table.render(option);
    ...
    table.on('tool(list)', function (obj) {
        ...
        if (obj.event === 'edit') {
            // 编辑功能
            WeAdminEdit('编辑','./userSignEdit.html',data.id);
        } else if (obj.event === 'del') {
            //删除功能
            js_tools.quick_post("/admin/api/userSign/delete",data,function (rs) {
    ...
    form.on('submit(search)', function(data){
        ...
        userSignListIns.reload({where:data.field});
