...
layui.use(['laydate','admin','form', 'jquery', 'table', 'js_tools'], function () {
    ...
    option.url = "/admin/api/sysInfoType/page";
    option.cols = [[
        {fixed: 'left',checkbox: true},
        {field: 'id', title: '资讯类型id'},
        {field: 'name', title: '资讯类型'},
        {fixed: 'right',title:'操作', width:150, align:'center', toolbar: '#list_btn'} //这里的toolbar值是模板元素的选择器
    ]];

    window.sysInfoTypeListIns = table.render(option);
    ...
    table.on('tool(list)', function (obj) {
        ...
        if (obj.event === 'edit') {
            // 编辑功能
            WeAdminEdit('编辑','./sysInfoTypeEdit.html',data.id);
        } else if (obj.event === 'del') {
            //删除功能
            js_tools.quick_post("/admin/api/sysInfoType/delete",data,function (rs) {
    ...
    form.on('submit(search)', function(data){
        ...
        sysInfoTypeListIns.reload({where:data.field});
