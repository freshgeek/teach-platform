...
layui.use(['laydate','admin','form', 'jquery', 'table', 'js_tools'], function () {
    ...
    option.url = "/admin/api/userAchvLike/page";
    option.cols = [[
        {fixed: 'left',checkbox: true},
        {field: 'id', title: 'id'},
        {field: 'userId', title: '用户id'},
        {field: 'achvId', title: '作品id'},
        {field: 'createTime', title: '点赞时间'},
        {fixed: 'right',title:'操作', width:150, align:'center', toolbar: '#list_btn'} //这里的toolbar值是模板元素的选择器
    ]];

    window.userAchvLikeListIns = table.render(option);
    ...
    table.on('tool(list)', function (obj) {
        ...
        if (obj.event === 'edit') {
            // 编辑功能
            WeAdminEdit('编辑','./userAchvLikeEdit.html',data.id);
        } else if (obj.event === 'del') {
            //删除功能
            js_tools.quick_post("/admin/api/userAchvLike/delete",data,function (rs) {
    ...
    form.on('submit(search)', function(data){
        ...
        userAchvLikeListIns.reload({where:data.field});
