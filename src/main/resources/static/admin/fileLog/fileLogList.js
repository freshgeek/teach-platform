...
layui.use(['laydate','admin','form', 'jquery', 'table', 'js_tools'], function () {
    ...
    option.url = "/admin/api/fileLog/page";
    option.cols = [[
        {fixed: 'left',checkbox: true},
        {field: 'id', title: '记录id'},
        {field: 'userId', title: '上传用户id'},
        {field: 'name', title: '上传文件名'},
        {field: 'size', title: '上传文件大小'},
        {field: 'path', title: '路径'},
        {field: 'createTime', title: '上传时间'},
        {fixed: 'right',title:'操作', width:150, align:'center', toolbar: '#list_btn'} //这里的toolbar值是模板元素的选择器
    ]];

    window.fileLogListIns = table.render(option);
    ...
    table.on('tool(list)', function (obj) {
        ...
        if (obj.event === 'edit') {
            // 编辑功能
            WeAdminEdit('编辑','./fileLogEdit.html',data.id);
        } else if (obj.event === 'del') {
            //删除功能
            js_tools.quick_post("/admin/api/fileLog/delete",data,function (rs) {
    ...
    form.on('submit(search)', function(data){
        ...
        fileLogListIns.reload({where:data.field});
