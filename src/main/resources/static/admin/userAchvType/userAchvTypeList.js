...
layui.use(['laydate','admin','form', 'jquery', 'table', 'js_tools'], function () {
    ...
    option.url = "/admin/api/userAchvType/page";
    option.cols = [[
        {fixed: 'left',checkbox: true},
        {field: 'id', title: '类别id'},
        {field: 'name', title: '作品类别名'},
        {field: 'sortField', title: '作品类别排序'},
        {fixed: 'right',title:'操作', width:150, align:'center', toolbar: '#list_btn'} //这里的toolbar值是模板元素的选择器
    ]];

    window.userAchvTypeListIns = table.render(option);
    ...
    table.on('tool(list)', function (obj) {
        ...
        if (obj.event === 'edit') {
            // 编辑功能
            WeAdminEdit('编辑','./userAchvTypeEdit.html',data.id);
        } else if (obj.event === 'del') {
            //删除功能
            js_tools.quick_post("/admin/api/userAchvType/delete",data,function (rs) {
    ...
    form.on('submit(search)', function(data){
        ...
        userAchvTypeListIns.reload({where:data.field});
