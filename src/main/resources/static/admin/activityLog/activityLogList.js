...
layui.use(['laydate','admin','form', 'jquery', 'table', 'js_tools'], function () {
    ...
    //执行一个laydate实例
    laydate.render({
        elem: '#commentTimeStart' //指定元素
        ,type: 'datetime'
    });

    //执行一个laydate实例
    laydate.render({
        elem: '#commentTimeEnd' //指定元素
        ,type: 'datetime'
    });

    var option = js_tools.getTableTemplate();
    option.url = "/admin/api/activityLog/page";
    option.cols = [[
        {fixed: 'left',checkbox: true},
        {field: 'id', title: '记录id'},
        {field: 'atvId', title: '项目id'},
        {field: 'userId', title: '参与者id'},
        {field: 'content', title: '内容'},
        {field: 'createTime', title: '记录时间'},
        {field: 'comment', title: '项目发起者评论'},
        {field: 'commentTime', title: '评论时间'},
        {fixed: 'right',title:'操作', width:150, align:'center', toolbar: '#list_btn'} //这里的toolbar值是模板元素的选择器
    ]];

    window.activityLogListIns = table.render(option);
    ...
    table.on('tool(list)', function (obj) {
        ...
        if (obj.event === 'edit') {
            // 编辑功能
            WeAdminEdit('编辑','./activityLogEdit.html',data.id);
        } else if (obj.event === 'del') {
            //删除功能
            js_tools.quick_post("/admin/api/activityLog/delete",data,function (rs) {
    ...
    form.on('submit(search)', function(data){
        ...
        activityLogListIns.reload({where:data.field});
